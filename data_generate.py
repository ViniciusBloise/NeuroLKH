...
def run(n_nodes, file_dir="train"):
    ...
    inverse_edge_index[np.arange(n_samples).reshape(-1, 1, 1), edge_index, np.arange(n_nodes).reshape(1, -1, 1)] = (
        np.arange(n_neighbours).reshape(1, 1, -1) + np.arange(n_nodes).reshape(1, -1, 1) * n_neighbours
    )
    ...
    os.chdir("../..")
    with open(Data_Dir + file_dir + "/" + str(n_nodes) + ".pkl", "wb") as f:
        pickle.dump(feat, f)
    if file_dir == "train":
        with open(f"walltime.txt", "a") as w:
            w.write(f"n={n_nodes} {(t_end - t_start) / 60:.2f}m ({n_samples} instances)\n")
