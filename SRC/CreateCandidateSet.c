...
void CreateCandidateSet()
{
        ...
        (CandidateSetType == QUADRANT || CandidateSetType == NN))
    {
            ...
            MaxCandidates > 0)
        {
            ...
            {
                if ((CoordType == TWOD_COORDS && Distance != Distance_TOR_2D) || (CoordType == THREED_COORDS && Distance != Distance_TOR_3D))
                    CreateNearestNeighborCandidateSet(MaxCandidates);
                else
    ...
    if ((PiFileName == 0) && (!ReadPenalties()))
    {
        ...
        CandidatesRead = ReadCandidates(MaxCandidates) ||
                         ReadEdges(MaxCandidates);
        Cost = Ascent();
        if (PiFileName != 0 && Subgradient && SubproblemSize == 0)
        {
            WritePenalties();
            PiFile = 0;
        }
    }
    else if ((PiFileName == 0) && ((CandidatesRead = ReadCandidates(MaxCandidates) ||
                                                     ReadEdges(MaxCandidates)) ||
                                   MaxCandidates == 0))
    {
        AddTourCandidates();
...
End_CreateCandidateSet:
         ...
         InitialTourAlgorithm != MOORE))
    {
        ...
        if (PiFileName == 0 && !CandidatesRead && SubproblemSize == 0)
            WriteCandidates();
    }
    if (C == C_EXPLICIT)
    {
