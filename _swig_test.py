...
def infer_SGN(
    ...
    for i in range(math.ceil(dataset_edge_index.shape[0] / batch_size)):
        ...
        y_edges = y_edges[:, :, 1].reshape(actual_batch_size, dataset_node_feat.shape[1], 20)
        y_edges = np.argsort(-y_edges, -1)
        edge_index = edge_index.cpu().numpy().reshape(-1, y_edges.shape[1], 20)
        candidate_index = edge_index[
            np.arange(actual_batch_size).reshape(-1, 1, 1), np.arange(y_edges.shape[1]).reshape(1, -1, 1), y_edges
        ]
