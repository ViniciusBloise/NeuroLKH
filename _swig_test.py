...
def generate_feat(data, n_nodes, seed=1234):
    ...
    edge_feat = invec[n_nodes * n_edges: n_nodes *
                      n_edges * 2].reshape(1, -1, 20)
    inverse_edge_index = invec[n_nodes * n_edges *
                               2: n_nodes * n_edges * 3].reshape(1, -1, 20)
...
def infer_SGN(
    ...
    for i in range(math.ceil(dataset_edge_index.shape[0] / batch_size)):
        ...
        actual_batch_size = node_feat.shape[0]
        edge_feat = edge_feat.view(actual_batch_size, -1, 1)
        edge_index = edge_index.view(actual_batch_size, -1)
        inverse_edge_index = inverse_edge_index.view(actual_batch_size, -1)
