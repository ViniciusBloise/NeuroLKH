...
class EdgeFeatures(nn.Module):
    ...
    def forward(self, x, e, edge_index, inverse_edge_index, n_edges):
        ...
        inverse_Ue = torch.cat((inverse_Ue, self.W_placeholder.view(1, 1, hidden_dim).repeat(batch_size, 1, 1)),
                               1)  # batch_size x (n_node * n_edge + 1) x hidden_dimension
...
class SparseGCNLayer(nn.Module):
    ...
    def forward(self, x, e, edge_index, inverse_edge_index, n_edges):
        ...
        e_tmp = self.edge_feat(x_new, e_in, edge_index.long(), inverse_edge_index.long(),
                               n_edges)  # batch_size x n_node * n_edge x hidden_dimension
